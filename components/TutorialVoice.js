...
const TutorialVoice = ({ onClose }) => {
  const [step, setStep] = useState(0); // Controla os passos do tutorial

  const steps = [
    {
      target: '.voice-channel-list', // Seleciona a lista de canais de voz
      content: 'Aqui você pode ver a lista de canais de voz disponíveis.',
    },
    {
      target: '.join-channel-button', // Seleciona o botão de entrar no canal de voz
      content: 'Clique aqui para entrar em um canal de voz.',
    },
    {
      target: '.leave-channel-button', // Seleciona o botão de sair do canal de voz
      content: 'Quando conectado e quiser sair do canal de voz, aparecerá um botão aqui para encerrar a chamada.',
    },
  ];
  ...
  const getHighlightStyle = () => {
    switch (currentStep.target) {
      case '.voice-channel-list':
        return {
          top: 240, // Ajuste a posição conforme necessário
          left: 10,
          width: 300,
          height: 400,
      ...
      case '.join-channel-button':
        return {
          top: 465, // Ajuste a posição conforme necessário
          left: 10,
          width: 300,
          height: 55,
          borderColor: 'blue',
      ...
      case '.leave-channel-button':
        return {
          top: 485, // Ajuste a posição conforme necessário
          left: 10,
          width: 160,
          height: 50,
          borderColor: 'red', // Cor vermelha da borda para o botão de sair
          borderWidth: '2px',
          borderStyle: 'solid',
        };
      default:
